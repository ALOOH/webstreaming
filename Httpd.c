...
#define MSG_EXCCEED_CHANNEL_CNT  "Err-ExcceedChannelCount\n"
...
extern int sendData(int fp, int channelNum);
...
void *client_thread(void *arg)
{
	...
	DBG("client Thread");
	printf("client Thread111\n");

	
	if (pthread_getschedparam(pthread_self(), &policy, &param) == 0)
			printf("#proicy[%d] priority = %d\n", policy, param.sched_priority);
...
*/
	...
	char daqId[16]={0,};
        ...
        if((cnt = _readline(lcfd.fd, &iobuf, buffer, sizeof(buffer) - 1, 86400)) == -1) {
	        DBG("readLine error\n");
			if(iobuf.buffer != NULL)
			{
		DBG("readLine iobuf : %s level : %d\n", iobuf.buffer, iobuf.level);					
//		DBG("readLine iobuf : %s level : %d\n", iobuf.buffer, iobuf.level);							
			}
//		DBG("readLine iobuf : %p\n", &iobuf);		
//		
///		DBG("readLine buffer : %p\n", buffer);		
//		DBG("readLine buffer : %p\n", lcfd.fd);
...
/*
	 ...
	 if(strstr(buffer, REQ_CMD_READDATA) != NULL) {
	 	char* pChannel;
		char channel[3]={0,};
		int nChannel=-1;
		int sem_index = 0;
		...
		/*
 			if(nStreamCnt > 3)
			{
				write(lcfd.fd, MSG_EXCCEED_CHANNEL_CNT, strlen(MSG_EXCCEED_CHANNEL_CNT));
				return NULL;
			}
	...
	*/
	nChannel = 4;
			...
			if(nChannel >0 && nChannel < 5)
			{
				...
				//chchoi_single
				//queue = sendData(lcfd.fd, nChannel);
				queue = sendData(lcfd.fd, 4);
				close(lcfd.fd);
				free_request(&req);
				nStreamCnt =0;
				break;
...
******************************************************************************/
extern void readADDataThread(void* arg);
...
//    pglobal = pcontext->pglobal;
    ...
    for(aip2 = aip; aip2 != NULL; aip2 = aip2->ai_next) {
        ...
        if(setsockopt(sd[i], IPPROTO_TCP, TCP_NODELAY, &on, sizeof(on)) < 0) {
            perror("setsockopt(SO_REUSEADDR) failed");
        }
