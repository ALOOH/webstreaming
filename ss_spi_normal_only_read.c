...
int streamingData(int socket_fd, int channel_fd) 
{
  int i, j, ret; 
//  int channel_fd;
#ifdef DEBUG	
...
int mode;
  ...
  sampleNumber =  requestCnt(channel_fd); 
  	...
  	//DBG(" Channel %d ,channelFd %d,  SampleCnt : %d\n", channelIndex, channel_fd, sampleNumber); 
#endif 
...
//jonsama end
  ...
  { 
	...
	{
 		printf(" %d sampleNumbe channel_fd:%d \n", sampleNumber, channel_fd); 		
	}
...
//				if( streamingData(lcfd.fd, nChannel ) == 0)
	//				 break;
				AD_EnableModule(1);			
				streamingData(NULL, fd1 );				
				AD_DisableModule(1);			

				AD_EnableModule(2); 		
				streamingData(NULL, fd2 );				
				AD_DisableModule(2);			

				AD_EnableModule(3); 		
				streamingData(NULL, fd3 );				
				AD_DisableModule(3);			

				AD_EnableModule(4); 		
				streamingData(NULL, fd4 );				
				AD_DisableModule(4);			
			printf("while end\n");
				//if(sem_index == 3)
				//	sem_post(&sem[0]);
